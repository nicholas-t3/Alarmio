...
//
// Usage:
//   ELEVENLABS_API_KEY=sk_... deno run -A scripts/generate-voice-previews.ts
//
...
async function synthesize(voice: Voice, apiKey: string): Promise<Uint8Array> {
...
async function main() {
    ...
    for (const voice of VOICES) {
        ...
        try {
            ...
            console.log(`  ✗ ${voice.key.padEnd(22)} ${(e as Error).message}`);
            failed++;
        }
    }
